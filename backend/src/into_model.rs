...
impl<'input> IntoModel for ServiceBody<'input> {
    ...
    fn into_model(self, scope: &Scope) -> Result<Self::Output> {
        ...
        return Ok(RpServiceBody {
            ...
            local_name: self.name.to_string(),
            comment: self.comment.into_iter().map(ToOwned::to_owned).collect(),
            endpoints: self.endpoints.into_model(scope)?,
            decls: vec![],
        });
    }
}

impl<'input> IntoModel for ServiceEndpoint<'input> {
    type Output = RpServiceEndpoint;

    fn into_model(self, scope: &Scope) -> Result<Self::Output> {
        let id = self.id.into_model(scope)?;

        let alias = self.alias.into_model(scope)?.or_else(|| {
            scope.endpoint_naming().map(|n| n.convert(id.as_str()))
        });

        return Ok(RpServiceEndpoint {
            id: id,
            alias: alias,
            comment: self.comment.into_iter().map(ToOwned::to_owned).collect(),
