...
use backend::errors::*;
use core::{RpChannel, RpServiceEndpoint};
    ...
    fn into_tokens(self) -> Tokens<'el, Java<'el>> {
        ...
        class.methods.push({
            ...
            m.body.push({
                ...
                t.push("try {");
                t.nested(toks!["return this.mapper.readValue(stream, this.", ty.var(), ");"]);
...
impl Module {
    ...
    fn method_type(&self, endpoint: &RpServiceEndpoint) -> MethodType {
        use self::RpChannel::*;
        ...
        field.initializer({
            ...
            init.nested({
                ...
                t.push(toks![
                    ...
                    service_name.quoted(), ", ", endpoint_name.quoted(),
                    "))",
                ]);
    ...
    fn client_stub<'el>(&self) -> Class<'el> {
        ...
        spec.methods.push({
            ...
            m.body.push(toks!["return new ", name.clone(), "(channel, callOptions);"]);
    ...
    fn server_stub<'el>(&self) -> Class<'el> {
        ...
        let mut spec = Class::new("ServerStub");
        spec.annotation(Generated(self, "Generated by ReProto".into()));
...
impl Listeners for Module {
    ...
    fn service_added(&self, e: &mut ServiceAdded) -> Result<()> {
        ...
        bind_service.body.nested(toks![".builder(", service_name.clone().quoted(), ")"]);

        for (endpoint, name) in e.body.endpoints.iter().zip(
